...
namespace KYM
{
    public class AbortState : AIStateBase
    {
        ...
        public override void onEnterState(AIBrain brain)
        {
            ...
            Vector3 abortDestination = brain.transform.position + new Vector3(abortDirection.x, 0, abortDirection.z) * 50.0f; // 도망갈 목적지 설정 
        ...
        {
            elapsedTime += Time.deltaTime; // 경과 시간 업데이트ccc

            if (elapsedTime >= abortTime)
            {
                // 도망가는 시간이 끝나면 기본 상태(예: PatrolState)로 전환
                brain.ChangeState(brain.StateMap[AIStateType.Patrol]);
             }
        }
